import { printRaw } from "./latex-parser";
...
const match = {
    ...
    environment(node, envName) {
        ...
        if (node.type === "environment" && printRaw(node.env) === envName) {
            return true;
        }
        return false;
    },
...
export function splitOnMacro(ast, macroName) {
    if (!Array.isArray(ast)) {
        console.warn(
            "Trying to split a non-array ast",
            ast,
            "by a macro named",
            macroName
        );
 ...
 * with the specified `processBody` function (if specified). Any specified `renderInfo`

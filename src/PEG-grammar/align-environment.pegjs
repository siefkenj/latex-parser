...
body = (comment_only_line / row_with_end / row_without_end)+ / EOL { return [] }

row_with_end = 
    // If there is a row ending we can have zero items
	  rowItems:(
    	x:row_items? { return {cells: [], colSeps: [], ...x} }
    )
    rowSep:row_sep 
...
comment_only_line = (whitespace* x:own_line_comment {
		return {
        	cells: [],
            colSeps: [],
            rowSep: null,
...
token = (!(row_sep / col_sep / trailing_comment / own_line_comment) x:. {return x})
cell = $token+
separated_cell 
